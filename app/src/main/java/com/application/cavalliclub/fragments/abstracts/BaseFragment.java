package com.application.cavalliclub.fragments.abstracts;
...
import com.application.cavalliclub.helpers.GPSTracker;
import com.application.cavalliclub.helpers.ServiceHelper;
...
import com.application.cavalliclub.retrofit.WebServiceFactory;
...
public abstract class BaseFragment extends Fragment implements webServiceResponseLisener {
	...
	public String getAllFilters(){
		...
		String commaSepratedString = android.text.TextUtils.join(",", allData);
	...
	public Integer getCartCount(){
		...
		RealmResults<AddCartEntity> results= realm.where(AddCartEntity.class).findAll();
		for(AddCartEntity item: results){
			count=count+item.getProductQuantity();
		}
		return count;
	}
}
