package com.application.cavalliclub.ui.views;
...
import com.application.cavalliclub.entities.MenuCategoryParentEntity;
...
import com.application.cavalliclub.ui.binders.CategoryItemBinder;
...
public class TitleBar extends RelativeLayout implements RecyclerViewItemListener {
    ...
    private ArrayList<EntityMemberships> membershipCollection;
    ...
    public void setRecyclerview_below() {
        RelativeLayout.LayoutParams params= new RelativeLayout.LayoutParams(ll_recyclerView.getLayoutParams());
        params.addRule(RelativeLayout.BELOW, R.id.title_buttons);
        ll_recyclerView.setLayoutParams(params);
    }
    ...
    public void setRecyclerViewData(DockActivity dockActivity, ArrayList<MenuCategoryParentEntity> menuCategoryParentEntities, MenuInterface menuInterface) {
    ...
    public void setMembershipRecyclerData(DockActivity dockActivity, ArrayList<EntityMemberships> memberships, MenuInterface menuInterface) {

        ll_recyclerView.setVisibility(VISIBLE);
        previousSelectedPos=0;

        userCollections = new ArrayList<>();
        membershipCollection = new ArrayList<>();
        membershipCollection.addAll(memberships);
        ...
        for(int i=0;i<=memberships.size()-1;i++){
            if(i==0){
                userCollections.add(new CategoriesEnt(memberships.get(i).getTitle() + "",true));
            }else{
                userCollections.add(new CategoriesEnt(memberships.get(i).getTitle() + "",false));
            }
        }
    ...
    public void showMembershipRecyclerDataVisible() {
    ...
    public void onEditItemClicked(Object Ent, int position, DockActivity dockActivity) {
       ...
       if(dataCollection!=null && dataCollection.size()>0) {
           menuInterface.menuItemClick(dataCollection.get(position),position);
       }else if(membershipCollection!=null && membershipCollection.size()>0){
           menuInterface.menuItemClick(membershipCollection.get(position),position);
       }
    }
